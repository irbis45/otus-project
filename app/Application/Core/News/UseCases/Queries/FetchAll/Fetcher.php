...
namespace App\Application\Core\News\UseCases\Queries\FetchAll;
...
class Fetcher
{
    ...
    public function fetch(): ResultDTO
    {
        ...
        $authorsIds = array_map(static fn (News $newsItem) => $newsItem->getAuthorId(), $newsItems);
        $categoryIds = array_map(static fn (News $newsItem) => $newsItem->getCategoryId(), $newsItems);

        $authors = $this->userRepository->findByIds($authorsIds);
